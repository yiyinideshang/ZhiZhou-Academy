...
namespace Maticsoft.DAL
{
	...
	public partial class Orders
	{
		...
		public bool Add(Maticsoft.Model.Orders model)
		{
			...
			strSql.Append("Id,OrderDate,UserId,TotalPirce,beiyong)");
			...
			SqlParameter[] parameters = {
					...
					new SqlParameter("@TotalPirce", SqlDbType.Decimal,9),
					new SqlParameter("@beiyong", SqlDbType.NVarChar,-1)};
			parameters[0].Value = model.Id;
			parameters[1].Value = model.OrderDate;
			parameters[2].Value = model.UserId;
			parameters[3].Value = model.TotalPirce;
			parameters[4].Value = model.beiyong;
		...
		public bool Update(Maticsoft.Model.Orders model)
		{
			...
			strSql.Append("TotalPirce=@TotalPirce,");
			...
			parameters[2].Value = model.TotalPirce;
		...
		/// 得到一个对象实体
		/// </summary>
		public Maticsoft.Model.Orders GetModel(int Id)
		{
			
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select  top 1 Id,OrderDate,UserId,TotalPirce,beiyong from Orders ");
			strSql.Append(" where Id=@Id ");
			SqlParameter[] parameters = {
					new SqlParameter("@Id", SqlDbType.Int,4)			};
			parameters[0].Value = Id;
		...
		public Maticsoft.Model.Orders DataRowToModel(DataRow row)
		{
			...
			if (row != null)
			{
				...
				if(row["TotalPirce"]!=null && row["TotalPirce"].ToString()!="")
				{
					model.TotalPirce=decimal.Parse(row["TotalPirce"].ToString());
				}
				if(row["beiyong"]!=null)
				{
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select Id,OrderDate,UserId,TotalPirce,beiyong ");
